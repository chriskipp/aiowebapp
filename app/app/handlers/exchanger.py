...
class DataHandler(BaseHandler):
    async def fileUploadFormHandler(self, request):
        ...
        response = aiohttp_jinja2.render_template(
            "uploadFiles.html", request, context=context
        )
        return response
            ...
            if part.name == "fileUpload":
                ...
                with open(uploadPath, "wb") as f:
                    while True:
                        ...
                        size += len(chunk)
                        f.write(chunk)
                response += "\n{} sized of {} successfully stored".format(
                    filename, size
                )
    ...
    def configure(self, app):
        ...
        router = app.router
        router.add_route(
            "GET", "/upload", self.fileUploadFormHandler, name="fileUpload"
        )
        router.add_route(
            "POST", "/upload", self.fileUploadHandler, name="upload"
        )
